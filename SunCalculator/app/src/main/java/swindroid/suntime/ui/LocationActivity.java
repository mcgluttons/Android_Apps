...
public class LocationActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            Log.d("PUPPY", getFileStreamPath(fileName).getAbsolutePath());

            if(!file.exists()) {
                // read from raw and write to file
                Log.d("PUPPY", "Creating new file from raw data");
                ...
                Log.d("PUPPY", "opening existing file from internal storage");
                ArrayList<String[]> contents = getFileContents(fileName);
                Log.d("PUPPY", "Contents size: " + contents.size());
            ...
            Log.d("PUPPY", "Something went wrong");
            e.printStackTrace();
        }
    }
    ...
    public ArrayList<String[]> getFileContents(String fileName) {
        try {
            ...
            Log.d("PUPPY", "lineData size: " + lineData.size());

            BufferedReader readFile = new BufferedReader(new InputStreamReader(openFileInput(fileName)));

            while((line = readFile.readLine()) != null) {
                Log.d("PUPPY", "line value: " + line);
                temp = line.split(",");
                lineData.add(temp);
                Log.d("PUPPY", "lineData incrementing size: " + lineData.size());
            }
            readFile.close();
            Log.d("PUPPY", "lineData final size: " + lineData.size());
            return lineData;
        } catch (Exception e) {
            Log.d("PUPPY", "Can't read from file");
            e.printStackTrace();
        }
        return null;
    }
